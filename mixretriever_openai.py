...
class MixPyseriniOpenaiSearcher:
    '''
    ...
    def _search_psg_embedding(self, query_embedding):
    ...
    def _search_stc_embedding(self, query_embedding):
    ...
    def _search_mg_embed(self, query_embedding, alpha,beta,l):
        if self.ds_id == 'arguana':
            dummy = self._search_psg_embedding(query_embedding)
            ...
            psg_res = self._search_psg_embedding(query_embedding).head(5)
            stc_res = self._search_stc_embedding(query_embedding).head(5)
    ...
    def search_query(self, query, query_embedding, weight, idfthreshold, head_num):
        self._search_basic_bm25(query)
        self._search_mg_embed(query_embedding,alpha=80,beta=40,l=0.1)
    ...
    def search_queries(self, query_path, output_path, head_num=10):
        ...
        for i in range(qdf.shape[0]):
            ...
            idxs, texts = self.search_query(query,q_embed,0.7,0.6,head_num)
